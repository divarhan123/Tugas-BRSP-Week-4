#Modul: Analisis Biomarker Kanker Pankreas dari Sampel Saliva Tikus
#Dataset: GSE47811 (Pancreatic Cancer vs Normal)
#Platform: Microarray 
...
BiocManager::install("mouse4302.db", ask = FALSE, update = FALSE)
...
gset <- getGEO("GSE47811", GSEMatrix = TRUE, AnnotGPL = TRUE)[[1]]
...
config$n_neighbors <- 5   # harus < jumlah sampel (12)

#Jalankan UMAP
umap_result <- umap(umap_input, config = config)
...
ggplot(volcano_data, aes(x = logFC, y = -log10(adj.P.Val), color = status)) +
  ...
  theme_minimal() +
  ggtitle("Volcano Plot DEG Kanker Pankreas")
...
#Pilih 5 gen paling signifikan berdasarkan adj.P.Val
topTableResults <- topTableResults[
  order(topTableResults$adj.P.Val),
]

top5 <- head(topTableResults, 5)

#Ambil matriks ekspresi untuk gen terpilih
mat_heatmap <- ex[top5$PROBEID, ]

#Gunakan Gene Symbol (fallback ke Probe ID)
gene_label <- ifelse(
  is.na(top5$SYMBOL) | top5$SYMBOL == "",
  top5$PROBEID,      # jika SYMBOL kosong → probe ID
  top5$SYMBOL        # jika ada → gene symbol
)
...
write.csv(topTableResults, "Hasil_GSE47811_DEG.csv")
